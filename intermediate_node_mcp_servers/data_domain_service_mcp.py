...
Root MCP
   ↓
Data Domain MCP (this server)
   ↓
Service MCP (file / transform)
"""
...
from data_domain_agent import root_agent
...
# Create MCP server
mcp = FastMCP("data-domain-router")
...
@mcp.tool()
async def route_data_query(prompt: str) -> str:
    """
    Route a user query to the appropriate downstream MCP service.

    PURPOSE
    -------
    This tool forwards the user's query to the Data Domain routing agent.
    The agent analyzes the query and determines which service MCP should
    handle the request.

    The Data Domain MCP does NOT execute tools itself.
    ...
    str
...

        {
          "status": "success",
          "node_type": "leaf",
          "service_name": "transform_service",
          "endpoint": "http://localhost:10000/transform-service-mcp/mcp",
          "description": "Converts data between formats. Supports CSV to JSON, JSON to CSV, and JSON to XML transformations.",
          "confidence": 0.95,
          "reason": "The query was routed to transform_service."
        }
    ...
    Example flow:
        ...
        Client → Data Domain MCP
                → route_data_query("convert csv to json")

        Data Domain MCP → Agent
                        → returns service endpoint with confidence 0.95

        Client → Transform Service MCP (http://localhost:10000/transform-service-mcp/mcp)
                → tools/list → csv_to_json()

    ARCHITECTURE ROLE
    -----------------
    This server represents the Data Domain layer in the MCP routing graph.
