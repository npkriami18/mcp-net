#!/usr/bin/env python3
"""Utility Domain MCP Router Server
...
Root MCP
   ↓
Utility Domain MCP (this server)
   ↓
Service MCP (transform / analysis / compute)
"""
...
from utility_domain_agent import root_agent
...
# Create MCP server
mcp = FastMCP("utility-domain-router")
...
def _run_agent_sync(prompt: str) -> str:
    ...
    prompt : str
        ...
        Examples:
            "convert this csv dataset to json"
        ...
        try:
            ...
            loop.run_until_complete(
                session_service.create_session(
                    app_name="utility-domain-router",
...
@mcp.tool()
async def route_utility_query(prompt: str) -> str:
    """
    Route a user query to the appropriate downstream MCP service.

    PURPOSE
    -------
    This tool forwards the user's query to the Utility Domain routing agent.
    The agent analyzes the query and determines which service MCP should
    handle the request.

    The Utility Domain MCP does NOT execute tools itself.
    ...
    str
...

        {
          ...
          "description": "Performs mathematical computations including arithmetic operations, averages, and matrix calculations.",
          "confidence": 0.95,
          "reason": "The query was routed to compute_service."
        }
    ...
    Example flow:
        ...
        Client → Utility Domain MCP
                → route_utility_query("calculate average of numbers")

        Utility Domain MCP → Agent
                           → returns service endpoint with confidence 0.95

        Client → Compute Service MCP (http://localhost:10000/compute-service-mcp/mcp)
                → tools/list → calculate_average()

    ARCHITECTURE ROLE
    -----------------
    This server represents the Utility Domain layer in the MCP routing graph.
